...
namespace MBTilesPlugin
{
    ...
    class MBTilesActionsDatabaseImpl : MBTilesActionsGenImpl
    {

        private Database db = null;

        public MBTilesActionsDatabaseImpl(string typepath, string url)
            : base()
        {
            directory = null;
            ...
            else if (typepath == null || typepath.Equals(ConstantMbTilePlugin.OPEN_TYPE_PATH_FULL))
            {
                if (url == null)
                {
                    url = Path.Combine(Windows.Storage.ApplicationData.Current.LocalFolder.Path, "tiles//");
                }
                directory = url;
            }
