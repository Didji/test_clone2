(function() {
    ...
    angular.module("smartgeomobile").filter("getIconFromModelOkeyForCensus", getIconFromModelOkeyForCensus);
    ...
    function getIconFromModelOkeyForCensus(Site, Project) {
        ...
        function _getIconFromModelOkeyForCensus(okey) {
            ...
            if (okey.search(/PROJECT_/) === 0) {
                var classIndex =
                    (Project.currentLoadedProject && Project.currentLoadedProject.getClassIndexForAddedAsset(okey)) ||
                    "0";
                return (
                    currentSymbology[okey + classIndex + ""] &&
                    currentSymbology[okey + classIndex + ""].style.symbol.icon
                );
            } else {
