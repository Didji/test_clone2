angular.module("smartgeomobile").directive("census", [
    "$compile",
    "ComplexAsset",
    "Icon",
    "i18n",
    ...
    "Project",
    "Utils",
    function($compile, ComplexAsset, Icon, i18n, $rootScope, Storage, G3ME, Camera, GPS, Site, Project, Utils) {
        "use strict";
            ...
            link: function(scope) {
                ...
                scope.save = function(update) {
                    if (!scope.root.isGeometryOk()) {
                        alertify.alert(
                            i18n.get("_CENSUS_ALERT_GEOM_") +
                                ' <span ng-if="metamodel[node.okey].is_graphical" style="background-color:#d9534f" class="badge"><span class="fa fa-map-marker"></span></span>'
                        );
                        return;
                    }
                ...
                scope.userLocationGeometry = function(node) {
                    GPS.getCurrentLocation(function(lng, lat) {
                        ...
                        node.layer = L.marker(node.geometry, {
                            icon: L.icon({
                                iconUrl:
                                    Site.current.symbology["" + node.okey + scope.defaultClassIndex].style.symbol.icon,
                ...
                scope.draw = function(node) {
                    ...
                    switch (Site.current.metamodel[node.okey].geometry_type) {
                        ...
                        default:
                            console.log(
                                "Type de géométrie non géré : " + Site.current.metamodel[node.okey].geometry_type
                            );
                            break;
                    }
                };
                ...
                scope.drawLine = function(node) {
                    function mouseClickHandler(e) {
                        ...
                        if (!scope.lastPointLayer) {
                            scope.lastPointLayer = new L.Circle(
                                clickLatLng,
                                (15 * 40075017 * Math.cos(L.LatLng.DEG_TO_RAD * clickLatLng[0])) /
                                    Math.pow(2, G3ME.map.getZoom() + 8),
                                {
                                    color: "#fc9e49",
                                    weight: 1,
                                    fillOpacity: 1
                            ...
                            scope.lastPointLayer.setRadius(
                                (15 * 40075017 * Math.cos(L.LatLng.DEG_TO_RAD * clickLatLng[0])) /
                                    Math.pow(2, G3ME.map.getZoom() + 8)
                            );
                        }
                ...
                scope.drawPolygon = function(node) {
                    ...
                    function mouseClickHandler(e) {
                        ...
                        if (!scope.firstPointLayer) {
                            scope.firstPointLayer = new L.Circle(
                                clickLatLng,
                                (15 * 40075017 * Math.cos(L.LatLng.DEG_TO_RAD * clickLatLng[0])) /
                                    Math.pow(2, G3ME.map.getZoom() + 8),
                                {
                ...
                scope.drawPoint = function(node) {
                    ...
                    function initializeNodeLayer(e) {
                        ...
                        var symbology =
                                Site.current.symbology["" + node.okey + scope.defaultClassIndex] ||
                                Site.current.symbology[
                                    "" + node.okey.replace("PROJECT_", "") + scope.defaultClassIndex
                                ],
                        ...
                        if (Site.current.metamodel[node.okey].angle) {
                            node.layer = L.orientedMarker(e.latlng, options)
                                .addTo(G3ME.map)
                                .activateOrientation();
                        } else {
                ...
                scope.confirmDelete = function(node) {
                    alertify.confirm(
                        "Supprimer " + node.fields[Site.current.metamodel[node.okey].ukey] + " ?",
                        function(yes) {
                            if (yes) {
                                node.delete();
                                scope.$apply();
