(function() {
    ...
    angular.module("smartgeomobile").factory("Asset", AssetFactory);
    ...
    function AssetFactory(G3ME, Marker, SQLite, Storage, $rootScope, $http, Site, GPS, Relationship, Right, Utils) {
        ...
        Asset.prototype.showOnMap = function(dontPlaceMarker) {
            ...
            this.onMap = true;
            if (!dontPlaceMarker) {
                this.consultationMarker =
                    this.consultationMarker ||
                    Marker.getMarkerFromAsset(this, function() {
                        self.zoomOn();
                    });
                if (G3ME.map) {
        ...
        Asset.prototype.fetchHistory = function() {
            ...
            $http
                .get(
                    Utils.getServiceUrl("gi.maintenance.mobility.history", {
                        id: this.guid,
                        limit: 5
                    })
                )
        ...
        Asset.findInBounds = function(center, bounds, callback) {
            ...
            for (var i = 0, length_ = Site.current.zones.length; i < length_; i++) {
                if (
                    G3ME.extents_match(Site.current.zones[i].extent, {
                        xmin: xmin,
                        xmax: xmax,
                        ymin: ymin,
                        ymax: ymax
            ...
            if (!zone || (G3ME.active_layers && !G3ME.active_layers.length)) {
                ...
                request +=
                    " and ( symbolId REGEXP '" + G3ME.active_layers.join("[0-9]+' OR symbolId REGEXP '") + "[0-9]+' )";
            }
        ...
        Asset.prototype.getLabel = function() {
            return this.label ||
                (this.okey &&
                    Site.current.metamodel[this.okey] &&
                    this.attributes[Site.current.metamodel[this.okey].ukey] &&
                    this.attributes[Site.current.metamodel[this.okey].ukey].length)
                ? this.attributes[Site.current.metamodel[this.okey].ukey]
                : (Site.current.metamodel[this.okey] && Site.current.metamodel[this.okey].label) || "";
        };
                    ...
                    for (var s in symbology) {
                        (function(s) {
                            ...
                            var args = [
                                ...
                                Site.current.symbology[symbolId].maxzoom,
                                symbolId
                            ];
                            tx.executeSql(
                                "UPDATE ASSETS SET minzoom = ?, maxzoom = ? WHERE symbolId = ?",
                                args,
                                function() {
        ...
        Asset.findAssetsByGuids = function(guids, callback, zones, partial_response) {
            ...
            SQLite.exec(
                zones[0].database_name,
                "SELECT * FROM ASSETS WHERE id in ( " + guids.join(",").replace(/[a-z0-9|-]+/gi, "?") + ")",
                guids,
                function(results) {
                ...
                Asset.findAssetsByGuids(guids, function(assets) {
                    ...
                    for (var i = 0; i < assets.length; i++) {
                        ...
                        if (project) {
                            ...
                            newAsset.symbolId =
                                "PROJECT_" + assets[i].okey + project.getClassIndexForUnchangedAsset(assets[i].okey);
                        }
        ...
        Asset.__buildRequest = function(asset_s, site) {
            ...
            for (i = 0; i < assets.length; i++) {
                ...
                values_in_request = [
                    ...
                    "" +
                        (asset.attributes[
                            (metamodel[asset.okey] || metamodel[asset.okey.replace("PROJECT_", "")]).ukey
                        ] || ""),
                    asset.maplabel || "",
                    +mySymbology.minzoom,
                    +mySymbology.maxzoom,
        ...
        Asset.findGeometryByGuids_big = function(guids, callback, partial_response) {
            ...
            if (guids.length === 0) {
                ...
                Asset.findGeometryByGuids(guids.slice(0, Asset.__maxIdPerRequest), function(assets) {
                    Asset.findGeometryByGuids_big(
                        guids.slice(Asset.__maxIdPerRequest),
                        callback,
                        partial_response.concat(assets)
                    );
                });
            }
        };
        ...
        Asset.findExtentByGuids = function(guids, callback, zones, partial_response) {
            ...
            SQLite.exec(
                zones[0].database_name,
                "SELECT min(xmin) as xmin, max(xmax) as xmax, min(ymin) as ymin, max(ymax) as ymax FROM ASSETS WHERE id in ( " +
                    guids.join(",").replace(/[a-z0-9|-]+/gi, "?") +
                    ")",
                guids.map(String),
                function(rows) {
        ...
        Asset.findAssetsByLabel = function(label, callback, zones, partial_response) {
            ...
            SQLite.exec(
                zones[0].database_name,
                "SELECT * FROM ASSETS WHERE label like ? limit 0, 10",
                ["%" + label + "%"],
                function(rows) {
                    for (var i = 0; i < rows.length; i++) {
        ...
        Asset.findAssetsByCriteria = function(search, callback, zones, partial_response, request) {
            ...
            if (!zones.length || partial_response.length >= Asset.__maxResultPerSearch) {
                ...
                if (
                    partial_response[0] &&
                    (!isNaN(parseFloat(partial_response[0].label)) && isFinite(partial_response[0].label))
                ) {
                    partial_response.sort(function(a, b) {
                        return parseFloat(a.label) < parseFloat(b.label) ? -1 : 1;
                    });
                ...
                for (var criter in search.criteria) {
                    if (search.criteria.hasOwnProperty(criter) && search.criteria[criter]) {
                        if (search.criteria[criter] instanceof Date) {
                            var day, month, year;
                            day =
                                search.criteria[criter].getDate().toString().length > 1
                                    ? search.criteria[criter].getDate().toString()
                                    : "0" + search.criteria[criter].getDate().toString();
                            month =
                                (search.criteria[criter].getMonth() + 1).toString().length > 1
                            ...
                            lowerCriteria = search.criteria[criter]
                                .toString()
                                .toLowerCase()
                                .replace("(", "\\(")
                                .replace(")", "\\)");
                        ...
                        var regex =
                            '.*"' +
                            criter.toString().toLowerCase() +
                            '":[("a-zA-Z\\s0-9^,)||(a-zA-Z\\s0-9^,)]*' +
                            lowerCriteria +
                            "[a-zA-Z\\s0-9^,]*";
