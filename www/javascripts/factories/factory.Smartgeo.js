(function() {
    ...
    angular.module("smartgeomobile").factory("Smartgeo", SmartgeoFactory);

    SmartgeoFactory.$inject = [
        "$rootScope",
        "$location",
        "$window",
        "Storage",
    ...
    function SmartgeoFactory($rootScope, $location, $window, Storage, Intents, LicenseManager, Authenticator) {
        ...
        Smartgeo._initialize = function() {
            Smartgeo._SMARTGEO_MOBILE_VERSION = $rootScope.version =
                window.smargeomobileversion +
                (window.smargeomobilebuild && window.smargeomobilebuild.length ? "-" + window.smargeomobilebuild : "");
            Smartgeo._SIDE_MENU_WIDTH =
                ($window.outerWidth || $window.screen.width) > 361
                    ? 300
