(function() {
    ...
    angular.module("smartgeomobile").factory("Project", ProjectFactory);
    ...
    function ProjectFactory(
        ...
        Project.prototype.load = function(callback) {
            ...
            $http
                .get(
                    Utils.getServiceUrl("project.mobility.load.json", {
                        id: this.id
                    })
                )
        ...
        Project.prototype.unload = function(callback) {
            ...
            $http
                .get(
                    Utils.getServiceUrl("project.mobility.unload.json", {
                        id: this.id
                    })
                )
                .success(function() {
                ...
                .error(Project.smartgeoReachError)
                .finally(function() {
        ...
        Project.prototype.synchronize = function(callback) {
            ...
            this.getSynchronizePayload(function(payload) {
                $http
                    .put(
                        Utils.getServiceUrl("project.mobility.save.json", {
                    ...
                    .error(Project.handleSynchronizeError)
                    .finally(function() {
        ...
        Project.prototype.getSynchronizePayload = function(callback) {
            ...
            Synchronizator.getAll("ComplexAsset", "project_new", function(newAssets) {
                ...
                Synchronizator.getAll("ComplexAsset", "project_update", function(updatedAssets) {
                    ...
                    Asset.findAssetsByGuids(project.added.concat(project.removed.concat(project.deleted)), function(
                        assets
                    ) {
        ...
        Project.checkRemoteLockedAssets = function() {
            $http.get(Utils.getServiceUrl("project.mobility.assets.locked")).success(function(guids) {
                Asset.lock(guids);
            });
        };
        ...
        Project.prototype.hasBeenModified = function() {
            return (
                this.new.length + this.deleted.length + this.updated.length + this.added.length + this.removed.length >
                0
            );
        };
        ...
        Project.prototype.serializeForSQL = function() {
            return [
                this.id,
                JSON.stringify(this),
                JSON.stringify(this.added),
                JSON.stringify(this.deleted),
                JSON.stringify(this.updated),
                JSON.stringify(this.new),
                this.loaded
            ];
        };
        ...
        Project.getLoadedProject = function(callback) {
            SQLite.exec(Project.database, "SELECT * FROM " + Project.table + " WHERE loaded = ? ", ["true"], function(
                rows
            ) {
        ...
        SQLite.exec(
            ...
            "CREATE TABLE IF NOT EXISTS " +
                Project.table +
                "(" +
                Project.columns.join(",").replace("id", "id unique") +
                ")"
        );
